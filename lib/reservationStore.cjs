...
function addFromExtract(raw) {
  ...
  const bokingDate = toText(raw.boking_date || raw.booking_date);
  const bokingTime = toText(raw.boking_time || raw.booking_time);
  const adultCount = toText(raw.adult_count);
  const childCount = toText(raw.child_count);
  const weddingTableCount = toText(raw.wedding_table_count);
...
module.exports = { addFromExtract, getRows };
